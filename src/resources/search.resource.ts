...
function registerResources(server: McpServer) {
	...
	server.resource(
		'search-coin',
		'Search for a coin by ticker or address, returning array of matching coins',
		async (uri: URL) => {
			const methodLogger = logger.forMethod('searchCoinResource');
			try {
				methodLogger.debug('Search coin resource called', {
					uri: uri.toString(),
				});
					...
					contents: [
						{
							...
							description: `Coin Search for ${uri.toString()}`,
						},
					],
				};
			} catch (error) {
